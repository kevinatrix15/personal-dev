...
{
  // TODO: FUTURE WORK- add more complete command line argument handling, with more robust
  // checking and a help option.
  ...
  // 3- simple path, following diagonal- circles at opposite corners from start and goal
  // 4- more complex path 
  // 5- Maze- staggered barriers
  ...
  switch (obstacleCase) {
    ...
    case obstacle_config::MAZE:
    {
      ...
      const size_t radius = static_cast<int>(std::min(nx, ny) / 10) - static_cast<int>(robotRadius);
  ...
  // Search for a solution, with the starting position at the bottom corner, and goal at opposite corner
